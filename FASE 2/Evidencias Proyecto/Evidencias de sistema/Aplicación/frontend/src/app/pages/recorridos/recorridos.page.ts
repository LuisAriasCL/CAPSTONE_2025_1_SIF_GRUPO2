...
export class HomePage implements OnInit, OnDestroy { // Implementa OnInit y OnDestroy
  ...
  ngOnInit() {
    ...
    this.navigationSubscription = this.router.events.pipe(
      ...
      console.log("[Mapa] NavigationEnd detectado para /recorridos. Params actuales:", currentParams);
      this.actualizarEstadoDesdeQueryParams(currentParams);
      if (this.map) { // Si el mapa ya está inicializado, reconfigurar la vista
        this.zone.run(async () => {
  ...
  private actualizarEstadoDesdeQueryParams(params: any) {
    ...
    if (nuevaAsignacionId !== this.asignacionIdContexto || nuevoVehiculoId !== this.vehiculoIdParaSeguir || nuevaRutaId !== this.rutaIdParaDibujar) {
        console.log(`[Mapa] actualizarEstadoDesdeQueryParams: Detectado cambio en IDs de seguimiento.`);
        ...
        console.log(`[Mapa] actualizarEstadoDesdeQueryParams: IDs de seguimiento no cambiaron.`);
    }
  }

  ionViewWillEnter() {
    console.log("[Mapa] ionViewWillEnter: Suscribiéndose a eventos de Socket y rooms.");
    // Lógica para unirse al room de Socket.IO si aplica
    if (this.socketService.isConnected()) {
      this.subscribeToAsignacionRoom();
    } else {
      console.log("[Mapa] Socket no conectado inicialmente, intentando conectar y suscribirse...");
      this.socketService.connect(); // Intenta conectar si el servicio lo gestiona así
      const connectSub = this.socketService.listen('connect').subscribe(() => { // <<--- CORREGIDO
        console.log("[Mapa] SocketService conectado (evento 'connect'), procediendo a suscribir a room.");
        this.subscribeToAsignacionRoom();
  ...
  private subscribeToAsignacionRoom() {
    if (this.asignacionIdContexto) {
      ...
      console.log(`[Mapa] Solicitud de suscripción a room asignacion_${this.asignacionIdContexto} enviada.`);
    }
  }
  ...
  ionViewDidEnter() {
    ...
    setTimeout(async () => {
      if (!this.map && this.mapContainerRef?.nativeElement) {
        console.log("[Mapa] ionViewDidEnter: Mapa no existe, llamando a initMap.");
  ...
  ngOnDestroy() {
    ...
    if (this.subscriptions && !this.subscriptions.closed) {
      ...
      console.log("[Mapa] ngOnDestroy: this.subscriptions (eventos de socket) desuscrito.");
    }
     // Desuscripción del room si aplica
    if (this.asignacionIdContexto) {
        this.socketService.emit('unsubscribeFromAsignacion', { asignacionId: this.asignacionIdContexto });
        console.log(`[Mapa] ngOnDestroy: Solicitud de desuscripción del room asignacion_${this.asignacionIdContexto} enviada.`);
    }
    if (this.map) {
  ...
  private async configurarVistaSegunQueryParams() {
    if (!this.map) {
        console.warn("[Mapa] configurarVistaSegunQueryParams: Mapa no está listo.");
        return;
    }
    console.log(`[Mapa] configurarVistaSegunQueryParams: VehiculoID=${this.vehiculoIdParaSeguir}, RutaID=${this.rutaIdParaDibujar}, AsigID=${this.asignacionIdContexto}`);
    ...
    if (this.vehiculoIdParaSeguir) {
      console.log(`[Mapa] Modo Seguimiento Específico para vehículo ${this.vehiculoIdParaSeguir}.`);
    ...
    if (this.map) {
      for (const idStr in this.vehicleMarkers) {
        ...
        if (id !== idVehiculoAMantener) {
          if (this.map.hasLayer(this.vehicleMarkers[id])) this.map.removeLayer(this.vehicleMarkers[id]);
          delete this.vehicleMarkers[id];
        }
      }
    }
    console.log(`[Mapa] Marcadores limpiados excepto para el vehículo ID: ${idVehiculoAMantener}.`);
  }
  ...
  private async initMap(): Promise<void> {
    ...
    console.log("[Mapa] initMap: Intentando inicializar mapa en contenedor:", mapContainer);
    try {
      const initialCoords: L.LatLngTuple = [-36.8201, -73.0443]; // Concepción, Chile
      const initialZoom = 13;
      this.map = L.map(mapContainer, { center: initialCoords, zoom: initialZoom, attributionControl: false }); // attributionControl false y añadirlo manualmente
  ...
  private async loadInitialVehicles(): Promise<void> {
    if (this.vehiculoIdParaSeguir && this.vehicleMarkers[this.vehiculoIdParaSeguir]) {
        console.log("[Mapa] loadInitialVehicles: Ya se sigue un vehículo, no se cargan todos.");
        return;
    }
    console.log("[Mapa] loadInitialVehicles: Cargando todos los vehículos (modo dashboard).");
    const sub = this.apiService.getVehicles().subscribe({
      next: (vehiculos: Vehiculo[]) => {
        console.log(`[Mapa] loadInitialVehicles: ${vehiculos.length} vehículos recibidos.`);
        this.zone.run(() => {
          vehiculos.forEach(vehiculo => {
            if (vehiculo.idVehi !== undefined && (!this.vehiculoIdParaSeguir || vehiculo.idVehi !== this.vehiculoIdParaSeguir)) {
  ...
  private async loadAndFocusSpecificVehicle(vehicleId: number): Promise<void> {
    try {
      ...
      if (vehiculo) {
        console.log(`[Mapa] loadAndFocusSpecificVehicle: Vehículo ID ${vehicleId} cargado.`);
        this.zone.run(() => {
          this.updateMarker(vehiculo); 
          if (vehiculo.latitud != null && vehiculo.longitud != null && this.map) {
            if (this.polylineRutaActual) { // Si hay una ruta, el fitBounds de la ruta tiene prioridad
                const vehicleLatLng = L.latLng(vehiculo.latitud, vehiculo.longitud);
                if (!this.map.getBounds().contains(vehicleLatLng)) { // Si el vehículo está fuera de la vista actual de la ruta
                    this.map.panTo(vehicleLatLng); // Centrar suavemente
                }
            } else { // Si no hay ruta, centrar en el vehículo
  ...
  private listenToSocketEvents(): void {
    ...
    const createSub = this.socketService.listen<Vehiculo>('vehicleCreated').subscribe(vehiculo => {
        if (!this.vehiculoIdParaSeguir || (vehiculo.idVehi !== undefined && vehiculo.idVehi === this.vehiculoIdParaSeguir)) {
            console.log('[Mapa] Evento Socket.IO [vehicleCreated] recibido:', vehiculo);
    ...
    const deleteSub = this.socketService.listen<{ id: number }>('vehicleDeleted').subscribe(data => {
        if (!this.vehiculoIdParaSeguir || data.id === this.vehiculoIdParaSeguir) {
            console.log('[Mapa] Evento Socket.IO [vehicleDeleted] recibido:', data);
    ...
    const simEndedSub = this.socketService.listen<any>('simulationEnded').subscribe(data => {
        if (this.asignacionIdContexto && data.asignacionId === this.asignacionIdContexto) {
            console.log('[Mapa] Evento Socket.IO [simulationEnded] recibido:', data);
    ...
    const simErrorSub = this.socketService.listen<any>('simulationError').subscribe(data => {
        console.error('[Mapa] Evento Socket.IO [simulationError] recibido:', data);
  ...
  private async dibujarRutaEnMapa(rutaId: number): Promise<void> {
    ...
    try {
      console.log(`[Mapa] dibujarRutaEnMapa: Cargando datos para ruta ID: ${rutaId}`);
      ...
      if (rutaData && rutaData.puntosRuta && Array.isArray(rutaData.puntosRuta) && rutaData.puntosRuta.length > 0) {
        ...
        if (coordenadasLeaflet.length > 0) {
            ...
            this.nombreRutaMostrada = rutaData.nombreRuta || `Ruta #${rutaData.idRuta}`; // Usar idRuta si está en la interfaz Route
            console.log(`[Mapa] Ruta "${this.nombreRutaMostrada}" (ID ${rutaId}) dibujada con ${coordenadasLeaflet.length} puntos.`);
        } else {
            this.nombreRutaMostrada = `${rutaData.nombreRuta || `Ruta ${rutaId}`} (Sin trazado válido)`;
            console.warn(`[Mapa] Coordenadas para ruta ID ${rutaId} están vacías después del filtrado/mapeo.`);
        }
      } else {
        this.nombreRutaMostrada = `Ruta ${rutaId} (No encontrada o sin puntos)`;
        console.warn(`[Mapa] No se pudo cargar el trazado para la ruta ID ${rutaId}. Respuesta API:`, rutaData);
      }
    } catch (error) {
  ...
  private updateMarker(vehiculoData: VehiculoConDatosSimulacion): void {
    if (!this.map) {
      console.warn(`[Mapa] updateMarker: Mapa no está listo. Vehículo: ${vehiculoData.patente || vehiculoData.idVehi}`);
      return;
    }
    const { idVehi, latitud, longitud } = vehiculoData;
    if (idVehi === undefined || latitud == null || longitud == null) {
      console.warn('[Mapa] updateMarker: Datos insuficientes (idVehi, latitud, longitud).', vehiculoData);
      return;
    }
  ...
  private removeMarker(vehicleId: number): void {
    ...
    if (this.map.hasLayer(marker)) { // Verificar si está en el mapa antes de remover
  ...
  private createPopupContent(vehiculo: VehiculoConDatosSimulacion): string {
    ...
    const latStr = vehiculo.latitud != null ? vehiculo.latitud.toFixed(6) : 'N/A';
    const lonStr = vehiculo.longitud != null ? vehiculo.longitud.toFixed(6) : 'N/A';
  ...
  clearTrackingAndReturnToList() {
    console.log("[Mapa] Limpiando seguimiento y volviendo a la lista de asignaciones.");
    if (this.asignacionIdContexto) {
        this.socketService.emit('unsubscribeFromAsignacion', { asignacionId: this.asignacionIdContexto });
    }
  ...
  ionViewDidLeave() {
    console.log("[Mapa] ionViewDidLeave: Solicitando desuscripción del room si aplica.");
    if (this.asignacionIdContexto) {
      this.socketService.emit('unsubscribeFromAsignacion', { asignacionId: this.asignacionIdContexto });
    }
