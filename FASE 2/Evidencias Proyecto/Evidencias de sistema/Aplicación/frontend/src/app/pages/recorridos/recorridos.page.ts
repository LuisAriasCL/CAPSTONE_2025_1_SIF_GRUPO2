...
export class HomePage implements OnInit, OnDestroy {
  ...
  // Observer para detectar cambios de visibilidad del mapa
  ...
  private procesarVehiculosConEstado(vehiculos: Vehiculo[]) {
    console.log(
      '[Recorridos] procesarVehiculosConEstado: Procesando vehículos',
      vehiculos.length
    );
    ...
    try {
      ...
      this.vehiculosEnRecorrido = this.vehiculosConEstado.filter(
        (v) => v.enRecorrido
      );
      this.vehiculosDisponibles = this.vehiculosConEstado.filter(
        (v) => !v.enRecorrido
      );
      ...
      console.error(
        '[Recorridos] procesarVehiculosConEstado: Error procesando vehículos',
        error
      );
  ...
  private determinarSiEstaEnRecorrido(vehiculo: Vehiculo): boolean {
    // Lógica para determinar si el vehículo está en recorrido
    // Esto puede basarse en si tiene coordenadas actualizadas recientemente
    // o si está asignado a una ruta activa
    return vehiculo.latitud != null && vehiculo.longitud != null;
  }

  private obtenerRutaActual(vehiculo: Vehiculo): string | undefined {
    // Lógica para obtener el nombre de la ruta actual
    // Esto puede basarse en asignaciones activas
    return undefined; // Por ahora retorna undefined
  }
