...
import { LoadingController, ToastController, NavController, ModalController } from '@ionic/angular/standalone';
import { ActivatedRoute, Router } from '@angular/router';
import { addIcons } from 'ionicons';
import { save, arrowBackOutline as ionArrowBackOutline, closeOutline } from 'ionicons/icons';
import { HttpErrorResponse } from '@angular/common/http';
import { AlertService } from '../../../core/services/alert.service';
import { ApiService, Vehiculo, TipoCombustibleVehiculo } from '../../../core/services/api.service';
import { BaseFormComponent } from '../../../core/base/base-form.component';
import { VehicleService } from '../../../core/services/vehicle.service';
import { VEHICLE_CONSTANTS, VEHICLE_ESTADO_LABELS, ENTITY_NAMES } from '../../../core/constants';
import { formatDate, currentDate } from '../../../core/utils';
import { FormUtils as NewFormUtils } from '../../../core/utils/form.utils';
import { VehicleStatusPipe } from '../../../shared/pipes/vehicle-status.pipe';
import { DataTableComponent, Column, PageEvent } from '../../../shared/components/data-table/data-table.component';
...
import { 
  ...
  eficienciaCombustibleValidator,
  latitudValidator,
  longitudValidator,
  FormUtils
} from '../../../core/utils';
...
const VEHICLE_FORM_CONFIG = {
  ...
  ESTADO_OPTIONS: VEHICLE_CONSTANTS.ESTADO_OPTIONS,
} as const;

// Interface for tab type safety
type TabType = typeof VEHICLE_FORM_CONFIG.TABS[keyof typeof VEHICLE_FORM_CONFIG.TABS];

@Component({
  selector: 'app-vehicle-form',  templateUrl: './vehicle-form.page.html',
...
export class VehicleFormPage implements OnInit {
  ...
  public selectedTab: TabType = VEHICLE_FORM_CONFIG.TABS.INFO_BASICA;
  public pageTitle = 'Nuevo Vehículo';
  public currentDate: string = currentDate;
  public FormUtils = FormUtils;
  // Static options (usando las nuevas constantes refactorizadas)
  public readonly constants = VEHICLE_CONSTANTS;
  public readonly estadosVehiculoOptions = VEHICLE_FORM_CONFIG.ESTADO_OPTIONS;
  ...
  compareWith = (o1: any, o2: any) => {
    return o1 === o2;
  };  // Función helper para mostrar etiquetas de estado más amigables (usando constantes refactorizadas)
  getEstadoLabel(estado: string): string {
    return VEHICLE_ESTADO_LABELS[estado as keyof typeof VEHICLE_ESTADO_LABELS] || estado;
  }
  // Método para obtener mensajes de validación usando las nuevas constantes
  getValidationMessage(fieldName: string): string | null {
    const field = this.vehicleForm.get(fieldName);
    return NewFormUtils.getFieldErrorMessage(field, fieldName, this.constants.VALIDATION_MESSAGES);
  }
  ...
  updatePageTitle() {
    ...
    this.vehicleForm = this.fb.group({
      ...
      patente: ['', [Validators.required, patenteValidator()]], 
      chasis: ['', [Validators.required, chasisValidator()]], 
      marca: ['', Validators.required], 
      modelo: ['', Validators.required], 
      anio: [null, [Validators.required, yearValidator(1970, new Date().getFullYear())]], 
      kmVehi: [0, [Validators.required, kilometrajeValidator(1000000)]], 
      fecAdqui: [currentDate, Validators.required], 
      estadoVehi: ['activo', Validators.required], 

      tipoVehi: ['Camioneta'], 
      tipoCombVehi: [null], 
      kmVidaUtil: [null, kilometrajeValidator(5000000)], 
      efiComb: [null, eficienciaCombustibleValidator()],
      latitud: [null, latitudValidator()],
      longitud: [null, longitudValidator()]
    });
  ...
  async loadVehicleData() {
    ...
    this.apiService.getVehicle(this.vehicleId).subscribe({
      ...
      error: async (err: HttpErrorResponse | Error) => { // Tipar err
        ...
        // Usar el servicio de alertas en lugar del componente directamente
        await this.alertService.showError(`No se pudieron cargar los datos del vehículo. ${message}`, 'Error');
  ...
  onIncidentesExport(format: string) {
    ...
    if (this.vehicleForm.invalid) {
      FormUtils.markFormGroupTouched(this.vehicleForm);
      this.presentToast('Por favor, completa los campos requeridos correctamente.', 'warning');
      return;
    }

    // Utilizamos el nuevo servicio de alertas para confirmaciones
    const confirmed = this.isEditMode 
      ? await this.alertService.confirmEdit(ENTITY_NAMES.VEHICLE)
      : await this.alertService.confirmCreate(ENTITY_NAMES.VEHICLE);
    ...
    saveObservable.subscribe({
      ...
      error: async (error: HttpErrorResponse | Error) => {
        ...
        // Utilizamos el nuevo servicio de alertas para errores
        await this.alertService.showError(`No se pudo guardar el vehículo. ${message}`, 'Error al Guardar');
      }
    });
  }
  async presentAlert(header: string, message: string) {
    // Utilizamos el nuevo servicio de alertas
  ...
  segmentChanged(event: any) {
    ...
    // pero deberíamos usar la función de utilidad en todas las instancias nuevas
