  ...
  try {
    const proximaPlanificacion = await VehiculoPlanificacion.findOne({
      ...
      include: [
        ...
        { model: PlanificacionMantenimiento, as: 'planificacion', attributes: ['desc_plan'] }
      ]
    });

    if (!proximaPlanificacion) {
      return { dias_restantes: null, descripcion: 'No hay mantenimientos programados.' };
    }
    ...
    const descripcionPlan = proximaPlanificacion.get('planificacion')?.get('desc_plan');
    const patenteVehiculo = proximaPlanificacion.get('vehiculo')?.get('patente');

    if (!fechaProxima || !descripcionPlan || !patenteVehiculo) {
        console.error("Dato inconsistente encontrado:", proximaPlanificacion.toJSON());
        return { dias_restantes: null, descripcion: 'Dato de planificación inconsistente.' };
    }
...
exports.getDashboardKpis = async (req, res) => {
  try {
    ...
    const vehiculosPorEstadoRaw = await Vehiculo.findAll({
      attributes: ['estado_vehi', [sequelize.fn('COUNT', sequelize.col('estado_vehi')), 'count']],
      group: ['estado_vehi'],
      raw: true,
    });
    ...
    const siniestrosMes = await Siniestro.count({ where: { fec_sini: { [Op.gte]: treintaDiasAtras } } });

    const totalKmRecorridosResult = await AsignacionRecorrido.findOne({
      attributes: [[sequelize.literal('SUM(km_fin_recor - km_ini_recor)'), 'totalKm']],
      where: { fec_fin_recor: { [Op.gte]: treintaDiasAtras }, estado_asig: 'completado' },
      raw: true,
    });
    ...
    const alertasMantenimientoPendiente = await OrdenTrabajo.count({ where: { estado_ot: { [Op.notIn]: ['completada', 'cancelada', 'rechazado'] } } });

    const alertasSiniestrosPendientes = await Siniestro.count({ where: { estado_sin: { [Op.in]: ['reportado', 'en_revision'] } } });
...
exports.getVehiculosPorTipo = async (req, res) => {
  try {
    const stats = await Vehiculo.findAll({
      attributes: ['tipo_vehi', [sequelize.fn('COUNT', sequelize.col('tipo_vehi')), 'count']],
      group: ['tipo_vehi'],
      raw: true,
    });
    const chartData = {
...
exports.getMantenimientosPorEstado = async (req, res) => {
  try {
    const stats = await OrdenTrabajo.findAll({
      attributes: ['estado_ot', [sequelize.fn('COUNT', sequelize.col('estado_ot')), 'count']],
      group: ['estado_ot'],
      raw: true,
    });
    const chartData = {
