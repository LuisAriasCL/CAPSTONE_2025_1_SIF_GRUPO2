...
router.post('/', async (req, res) => {
  ...
  try {
    let {
      fecIniRecor, fecFinRecor, kmIniRecor, notas,
      vehiculoIdVehi, usuarioIdUsu, rutaIdRuta, estadoAsig = 'asignado',
    } = req.body;
    ...
    if (!conductor || conductor.rol !== 'conductor') {
      await t.rollback();
      return res.status(404).json({ message: 'El usuario no es un conductor válido.' });
    }
    ...
    if (fecFinRecor) {
        const asignacionVehiculoExistente = await AsignacionRecorrido.findOne({
            where: {
                ...
                [Op.or]: [
                    ...
                    { [Op.and]: [{ fec_ini_recor: { [Op.lte]: fecIniRecor } }, { fec_fin_recor: { [Op.gte]: fecFinRecor } }] }
                ]
            },
            transaction: t
        });
        ...
        if (asignacionVehiculoExistente) {
            await t.rollback();
            return res.status(409).json({ message: `Conflicto: El vehículo ya está asignado a otro recorrido (ID: ${asignacionVehiculoExistente.idAsig}) en ese período.` });
        }
        ...
        if (asignacionConductorExistente) {
            await t.rollback();
            return res.status(409).json({ message: `Conflicto: El conductor ya está asignado a otro recorrido (ID: ${asignacionConductorExistente.idAsig}) en ese período.` });
        }
    }
...
router.put('/:idAsig', async (req, res) => {
  ...
  try {
    ...
    const fechaInicioFinal = datosActualizacion.fecIniRecor || asignacion.fecIniRecor;
    const fechaFinFinal = datosActualizacion.fecFinRecor || asignacion.fecFinRecor;
    const vehiculoIdFinal = datosActualizacion.vehiculoIdVehi || asignacion.vehiculoIdVehi;
    const conductorIdFinal = datosActualizacion.usuarioIdUsu || asignacion.usuarioIdUsu;

    if ((datosActualizacion.fecIniRecor || datosActualizacion.fecFinRecor || datosActualizacion.vehiculoIdVehi || datosActualizacion.usuarioIdUsu) && fechaFinFinal) {
        const asignacionVehiculoExistente = await AsignacionRecorrido.findOne({
          where: {
            ...
            estadoAsig: { [Op.notIn]: ['completado', 'cancelado'] },
            [Op.or]: [
              { fec_ini_recor: { [Op.between]: [fechaInicioFinal, fechaFinFinal] } },
              { fec_fin_recor: { [Op.between]: [fechaInicioFinal, fechaFinFinal] } },
              { [Op.and]: [{ fec_ini_recor: { [Op.lte]: fechaInicioFinal } }, { fec_fin_recor: { [Op.gte]: fechaFinFinal } }] }
            ]
          },
          transaction: t
        });
        ...
        if (asignacionVehiculoExistente) {
          await t.rollback();
          // ===== CORRECCIÓN AQUÍ =====
          return res.status(409).json({ message: `Conflicto: El vehículo ya tiene otra asignación (ID: ${asignacionVehiculoExistente.idAsig}) en esas fechas.` });
        }
        ...
        if (asignacionConductorExistente) {
            await t.rollback();
            // ===== CORRECIÓN AQUÍ =====
            return res.status(409).json({ message: `Conflicto: El conductor ya tiene otra asignación (ID: ${asignacionConductorExistente.idAsig}) en esas fechas.` });
        }
    }

    if (datosActualizacion.estadoAsig === 'completado') {
      const kmFinal = datosActualizacion.kmFinRecor;
      const kmInicial = asignacion.kmIniRecor;

      if (kmFinal === undefined || kmFinal === null || parseFloat(kmFinal) <= parseFloat(kmInicial)) {
        await t.rollback();
        return res.status(400).json({ message: 'Para completar, el kilometraje final es requerido y debe ser mayor al inicial.' });
      }
      ...
      await Vehiculo.update({ kmVehi: kmFinal }, { where: { id_vehi: asignacion.vehiculoIdVehi }, transaction: t });
    }
    ...
    const asignacionActualizada = await AsignacionRecorrido.findByPk(idAsig, {
        include: [
            { model: Vehiculo, as: 'vehiculo' }, { model: Usuario, as: 'conductor' }, { model: Ruta, as: 'rutaPlantilla' }
        ]
    });
    ...
    res.status(500).json({ message: 'Error interno del servidor al actualizar la asignación.' });
  }
});
